...
export default function ConsistencyIntelligenceReport({ disciplineScore, winRate, totalTrades, emotionalTrades, appLanguage }: Props) {
  ...
  return <div className="relative overflow-hidden rounded-[40px] border border-white/10 bg-gradient-to-br from-[#071018] via-[#0f1726] to-black p-10"><div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,color-mix(in_srgb,var(--color-accent-bright)_10%,transparent),transparent_35%)]" /><div className="relative z-10"><p className="text-sm uppercase tracking-[0.3em] text-accent-bright">{i.consistencyIntelligence}</p><h2 className="mt-4 text-5xl font-black tracking-tight text-white">{i.consistencyReport}</h2><div className="mt-10 grid grid-cols-1 gap-5 md:grid-cols-4">{[{ label: t.discipline, value: disciplineScore, cls: "text-accent" }, { label: t.winRate, value: `${winRate}%`, cls: "text-accent-bright" }, { label: i.emotionalRatio, value: `${Math.round(emotionalRatio * 100)}%`, cls: "text-yellow-400" }, { label: i.structure, value: level, cls: tone }].map(c => <div key={c.label} className="rounded-3xl border border-white/10 bg-black/20 p-6"><p className="text-sm text-gray-400">{c.label}</p><h3 className={`mt-4 ${c.label === i.structure ? "text-2xl" : "text-4xl"} font-black ${c.cls}`}>{c.value}</h3></div>)}</div><div className="mt-10 rounded-3xl border border-white/10 bg-black/20 p-6"><p className="text-sm text-gray-400">{i.aiConsistencyAssessment}</p><h3 className={`mt-4 text-3xl font-black ${tone}`}>{level}</h3><p className="mt-4 max-w-4xl text-base leading-relaxed text-gray-300">{message}</p></div></div></div>;
}
