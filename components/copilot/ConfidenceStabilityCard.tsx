...
export default function ConfidenceStabilityCard({ confidenceDecay, recentConfidenceAverage, previousConfidenceAverage, appLanguage }: Props) { const t = getCopilotLabels(appLanguage); return <div className="rounded-[36px] border border-accent-bright/20 bg-accent-bright/10 p-8"><div className="flex items-center justify-between"><div><p className="text-sm uppercase tracking-[0.2em] text-accent-bright">{t.components.confidenceStability.eyebrow}</p><h2 className="mt-3 text-3xl font-black text-white">{t.components.confidenceStability.title}</h2></div><Badge active={confidenceDecay} activeText={t.common.declining} inactiveText={t.common.stable} /></div><div className="mt-8 grid gap-4 xl:grid-cols-3"><Metric label={t.components.confidenceStability.recentConfidence} value={`${recentConfidenceAverage}/10`} /><Metric label={t.components.confidenceStability.previousConfidence} value={`${previousConfidenceAverage}/10`} /><Metric label={t.common.status} value={confidenceDecay ? t.common.declining : t.common.stable} tone={confidenceDecay ? "text-red-400" : "text-emerald-400"} /></div></div> }
