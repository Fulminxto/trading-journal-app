...
const active: LifecycleAccount = { id: "active-1", name: "Active account", status: "ACTIVE", type: "LIVE", currency: "USD", broker: "Broker", canArchiveOrRestore: true, canDelete: true };
const archived: LifecycleAccount = { id: "archived-1", name: "Archived account", status: "ARCHIVED", type: "PROP", currency: "EUR", broker: null, canArchiveOrRestore: true, canDelete: false };
...
describe("account lifecycle management", () => {
  ...
  it("renders lifecycle actions as accessible siblings without nested controls", () => {
    ...
    expect(markup).toContain('aria-label="Archive account: Active account"');
    expect(markup).toContain('aria-label="Restore account: Archived account"');
    expect(markup).toContain('aria-label="Delete account: Active account"');
    expect(markup).not.toMatch(/<button[^>]*>(?:(?!<\/button>)[\s\S])*<a(?:\s|>)/);
    expect(markup).not.toMatch(/<a(?:\s[^>]*)?>(?:(?!<\/a>)[\s\S])*<button\b/);
  });

  it("positions the page as lifecycle management and removes embedded configuration creation", () => {
    ...
    expect(source).toContain('href="/accounts/create"');
  ...
  it("keeps active and archived groups distinct and uses partial section messages", () => {
    const source = readFileSync("app/accounts/manage/page.tsx", "utf8");

    expect(source).toContain("activeAccounts.length > 0");
    expect(source).toContain("archivedAccounts.length > 0");
    expect(source).toContain("No manageable active accounts.");
    expect(source).toContain("No manageable archived accounts.");
    expect(source).toContain("lifecycleAccounts.length === 0");
  });
  ...
  it("keeps the server query authoritative and passes no integration secrets to the client", () => {
    ...
    expect(page).toContain("canCreateAccount &&");
